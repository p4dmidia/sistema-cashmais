...
-- Permitir atualização apenas do próprio registro (quando implementado)
CREATE POLICY "Permitir atualização do próprio perfil" ON affiliates
    FOR UPDATE
    USING (auth.uid() = id);
...
GRANT UPDATE ON affiliates TO authenticated;